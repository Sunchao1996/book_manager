...
<script type="text/javascript">
	...
	function displayData(pageNo){
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/manager/getByPage.do",
			type:"get",
			cache:false,
			data:{
				"pageNo":pageNo+1,
			...
			success:function(jsonObject){
				...
				if(jsonObject.total==0){					
					...
					$.each(jsonObject.dataList,function(i,n){
						...
						htmlString +='<td><input type="checkbox" name="id" value="'+n.userId+'" onclick="controlFirstCheckbox();"/></td>';
						htmlString +='<td>'+(i+1)+'</td>';
						htmlString +='<td>'+n.userId+'</td>';
						htmlString +='<td>'+n.userPassword+'</td>';						
						htmlString +='</tr>';
					});
	...
	function del(){
		...
		if(checkedElts.length==0){
			...
			if(confirm("您确定要删除选中的记录吗?")){
				...
				alert(sendData);
				$.ajax({
					url:"${pageContext.request.contextPath}/manager/delete.do",
					type:"post",
					data:sendData,
					beforeSend:function(){
						$("#message").text("正在删除请稍后...");
						return true;
					},
					success:function(jsonObject){
	...
	function repwd(){
		...
		if(checkedElts.length>1 || checkedElts.length == 0){
			...
			if(confirm("您确定要重置密码吗?")){				
				$.ajax({
					url:"${pageContext.request.contextPath}/manager/repwd.do",
					type:"post",
					data:{"userId":checkedElts.val()},
					beforeSend:function(){
				...
				<tr>
					<td align="center">
						<table border="0" cellpadding="0" cellspacing="0" class="box_table" id="box_table">
							<tr>
								<td width="22%">&nbsp;用户名：<input type="text" id="userId"/></td>
				...
				<tr>
					<td>
						<table style="position : relative;left : 5px; top : 5px;">
							<tr>								
								<td width="50px">
									<a class="button" href="javascript:void(0);" onclick="del();"><span>删除</span></a>
								</td>																
								<td width="75px">
